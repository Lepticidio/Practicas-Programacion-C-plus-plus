...
struct Exercise3 {
	...
	static vec3 getWorldMousePosition(float mouse_x, float mouse_y, float windowsWidth, float windowsHeight, const mat4& projMat, const mat4& viewMat) {
		...
		float fCubeX = (mouse_x / windowsWidth) * 2 - 1;
		float fCubeY = (-1)*((mouse_y / windowsHeight) * 2 - 1);
		vec3 vCubeCoords(fCubeX, fCubeY, -1);
		mat4 mInverseProjection = inverse(projMat);
		vec4 vCubeCoordsTetra(vCubeCoords, 1);
		vec4 vViewCoords = mInverseProjection * vCubeCoordsTetra;
		vViewCoords.w = 1;

		mat4 mInverseView = inverse(viewMat);
		vec4 vWorldCoords = mInverseView*vViewCoords;
		return vec3(vWorldCoords);
