...
struct Exercise1 {
	...
	void init(int width, int height) 
	{
		...
		camera.speed = 5.0f;
		camera.yaw_speed = 120.f; //velocidad de giro en y. En x es pitch? y en z es roll?

		cameraMatrix = identity_mat4();
		cameraPosition = vec3(3, 2, 7);
	...
	void update() 
	{
		...
		{
			cameraPosition.z = cameraPosition.z + elapsed_seconds * camera.speed;
		...
		if (glfwGetKey(window, GLFW_KEY_D))
		{
			...
			cubeRotation = rotate_z_deg(cubeRotation, -cubeRotationSpeed*elapsed_seconds);
			cubeMatrix = cubeMatrix * cubeRotation;
		}
