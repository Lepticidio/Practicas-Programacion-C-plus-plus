...
namespace file
{
	namespace utils
	{
		unsigned int SFileUtils::CountStringsInString(const char* _sSearchedString, const char* _sReadedString)
		{
		...
		unsigned int  SFileUtils::CountStringsInFile(const char* _sFileName, const char* _sString)
		{
			...
			if (pFile1 != nullptr)
			{
				unsigned int uReadBytes = ReadFile(sBuffer, uBUFFER_SIZE);
				printf("Bytes read: %d\n", uReadBytes);
				int iClosefileResult = CloseFile();
		...
		unsigned int  SFileUtils::AddIntsInString(const char* _sString)
		{
		...
		unsigned int  SFileUtils::SumIntsInFile(const char* _sFileName)
		{
		...
		TList  SFileUtils::MakeNumberListFromString(const char* _sString)
		{
			TList tResult;
			bool bSameNumber = false;
			...
			for (int i = 0; i < strlen(_sString); i++)
			{
				...
				else
				{
					if (bSameNumber)
					{
						char* sCurrent = new char[uStringSize + 1];
						for (int j = 0; j < uStringSize; j++)
						{
							sCurrent[j] = _sString[uFirstIndex + j];
						}
						sCurrent[uStringSize] = '\0';
						tResult.Push(sCurrent);
						uStringSize = 0;
						bSameNumber = false;
					}
		...
		void  SFileUtils::PrintNumbersInFile(const char* _sFileName)
		{
			const unsigned int uBUFFER_SIZE = 100;
			char sBuffer[uBUFFER_SIZE];
			void* pFile1 = OpenFile(_sFileName, "r");
			...
			TList tList = MakeNumberListFromString(sBuffer);
		...
		unsigned int  SFileUtils::GetFileSize(void* _pFile)
		{
